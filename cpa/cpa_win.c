...
#define _FOLD_ "D:\\choi\\Univ\\2024-2\\부채널분석\\"
...
#define startpoint 23537
#define endpoint 72608
...
void CPA() {
    ...
    sprintf_s(buf, sizeof(char) * 256, "%s%s", _FOLD_, AlignedTraceFN);
    if ((err = fopen_s(&rfp, buf, "rb"))) {
        printf("FILE OPEN ERROR\n");
    }
    fread(&TraceLength, sizeof(int), 1, rfp);
    ...
    sprintf_s(buf, sizeof(char) * 256, "%s%s", _FOLD_, PlaintextFN);
    ...
    for (i = 0; i < 16; i++) {
        ...
        for (key = 0; key < 256; key++) {
            ...
            sprintf_s(buf, sizeof(char) * 256, "%scorrtrace\\%02d_block_%02x.corrtrace", _FOLD_, i, key);
            if ((err = fopen_s(&wfp, buf, "wb"))) {
                printf("FILE OPEN ERROR\n");
                printf("FILE OPEN ERROR\n");
            }
